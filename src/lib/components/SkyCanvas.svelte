<script>
  ...
  // const magnitudeScale = scaleLinear().domain([-1.5, 8]).range([3, 0.1]);
  const magnitudeScale = scaleSqrt().domain([-1.5, 8]).range([5, 0.1]);
  /** @type {String} [stroke='#ccc'] - The shape's stroke color. */
  export let stroke = "#ccc";

  const lineWidthScale = scaleSqrt().domain([1, 15]).range([1, 0.05]);
  ...
  $: makeEllipse = (d) => {
    ...
    const angle =
      Math.atan2(centroid[1] - pole[1], centroid[0] - pole[0]) -
      (d.GALDIM_ANGLE * Math.PI) / 180;
